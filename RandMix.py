...
class RandMix(nn.Module):
    def __init__(self, noise_lv):
        ...
        self.adain_1 = AdaIN2d(zdim, 3).cuda()
        self.adain_2 = AdaIN2d(zdim, 3).cuda()
        self.adain_3 = AdaIN2d(zdim, 3).cuda()
        self.adain_4 = AdaIN2d(zdim, 3).cuda()
    ...
    def forward(self, x, estimation=False, ratio=0):
        ...
        weight = torch.randn(6).cuda()
        weight[5] = 1
        for i in range(5):
            weight[i] /= 5

        x = x + torch.randn_like(x) * self.noise_lv * 0.001
        x_c = torch.tanh(F.dropout(color(x), p=.2)).cuda()
